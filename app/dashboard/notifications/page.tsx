...
export default function NotificationsPage() {
  ...
  console.log("[NotificationsPage] Rendering with user:", user?.role, "isAuthenticated:", isAuthenticated)

  useEffect(() => {
    if (isAuthenticated && user) {
      console.log("[NotificationsPage] User authenticated, fetching notifications")
      fetchNotifications()
      fetchUnreadCount()
    } else {
      console.log("[NotificationsPage] User not authenticated yet, skipping fetch")
    }
  ...
  const fetchNotifications = async (page = 1, status?: 'read' | 'unread') => {
    try {
      ...
      const data = await ApiService.getFarmerNotificationsWithStatus(params)
  ...
  const fetchUnreadCount = async () => {
    try {
      const data = await ApiService.getFarmerUnreadNotificationsCount()
      setUnreadCount(data.unreadCount)
    } catch (err: any) {
      console.error("[NotificationsPage] Error fetching unread count:", err)
    }
  }
  ...
  const markAsRead = async (notificationId: string) => {
    try {
      ...
      await ApiService.markFarmerNotificationAsRead(notificationId)
  ...
  const markAsUnread = async (notificationId: string) => {
    try {
      setLoadingAction(notificationId)
      console.log("[NotificationsPage] Marking notification as unread:", notificationId)
      
      await ApiService.markFarmerNotificationAsUnread(notificationId)
      ...
      console.error("[NotificationsPage] Error marking as unread:", err)
  ...
  const markAllAsRead = async () => {
    try {
      ...
      const result = await ApiService.markAllFarmerNotificationsAsRead()
      ...
      console.log("[NotificationsPage] Marked", result.modifiedCount, "notifications as read")
    } catch (err: any) {
  ...
  const deleteNotification = async (notificationId: string) => {
    try {
      ...
      await ApiService.deleteFarmerNotification(notificationId)
  ...
  const clearReadNotifications = async () => {
    try {
      ...
      console.log("[NotificationsPage] Clearing read notifications")
      
      const result = await ApiService.clearFarmerReadNotifications()
      
      // Remove read notifications from local state
      setNotifications(prev => prev.filter(n => !(n.isRead || n.read)))
      
      console.log("[NotificationsPage] Cleared", result.deletedCount, "read notifications")
    } catch (err: any) {
      console.error("[NotificationsPage] Error clearing read notifications:", err)
  ...
  if (!isAuthenticated || !user) {
    console.log("[NotificationsPage] User not authenticated, showing loading state")
    return (
  ...
  if (loading) {
    console.log("[NotificationsPage] Still loading, showing loading state")
    return (
      <ProtectedRoute allowedRoles={["farmer"]}>
        <DashboardLayout title="Notifications">
          <div className="flex items-center justify-center h-64">
  ...
  console.log("[NotificationsPage] Rendering main content")
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-muted-foreground">Stay updated with your farm reports and alerts</p>
            </div>
