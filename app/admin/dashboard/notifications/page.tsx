...
export default function AdminNotificationsPage() {
  ...
  console.log("[AdminNotificationsPage] Rendering with user:", user?.role, "isAuthenticated:", isAuthenticated)

  useEffect(() => {
    if (isAuthenticated && user) {
      console.log("[AdminNotificationsPage] User authenticated, fetching notifications")
      fetchNotifications()
      fetchUnreadCount()
    } else {
      console.log("[AdminNotificationsPage] User not authenticated yet, skipping fetch")
    }
  ...
  const fetchNotifications = async (page = 1, status?: 'read' | 'unread') => {
    try {
      ...
      const data = await ApiService.getAdminNotificationsWithStatus(params)
      console.log("[AdminNotificationsPage] Notifications fetched:", data)
      ...
      console.error("[AdminNotificationsPage] Error fetching notifications:", err)
  ...
  const fetchUnreadCount = async () => {
    try {
      const data = await ApiService.getAdminUnreadNotificationsCount()
      setUnreadCount(data.unreadCount)
    } catch (err: any) {
      console.error("[AdminNotificationsPage] Error fetching unread count:", err)
    }
  }
  ...
  const markAsRead = async (notificationId: string) => {
    try {
      ...
      console.error("[AdminNotificationsPage] Error marking as read:", err)
  ...
  const markAsUnread = async (notificationId: string) => {
    try {
      setLoadingAction(notificationId)
      console.log("[AdminNotificationsPage] Marking notification as unread:", notificationId)
      
      await ApiService.markAdminNotificationAsUnread(notificationId)
      ...
      console.error("[AdminNotificationsPage] Error marking as unread:", err)
  ...
  const markAllAsRead = async () => {
    try {
      ...
      const result = await ApiService.markAllAdminNotificationsAsRead()
      ...
      console.log("[AdminNotificationsPage] Marked", result.modifiedCount, "notifications as read")
    } catch (err: any) {
      console.error("[AdminNotificationsPage] Error marking all as read:", err)
  ...
  const deleteNotification = async (notificationId: string) => {
    try {
      ...
      console.error("[AdminNotificationsPage] Error deleting notification:", err)
  ...
  const clearReadNotifications = async () => {
    try {
      ...
      console.log("[AdminNotificationsPage] Clearing read notifications")
      
      const result = await ApiService.clearAdminReadNotifications()
      
      // Remove read notifications from local state
      setNotifications(prev => prev.filter(n => !(n.isRead || n.read)))
      
      console.log("[AdminNotificationsPage] Cleared", result.deletedCount, "read notifications")
    } catch (err: any) {
      console.error("[AdminNotificationsPage] Error clearing read notifications:", err)
  ...
  if (!isAuthenticated || !user) {
    console.log("[AdminNotificationsPage] User not authenticated, showing loading state")
    return (
  ...
  if (loading) {
    console.log("[AdminNotificationsPage] Still loading, showing loading state")
    return (
      <ProtectedRoute allowedRoles={["admin"]}>
        <DashboardLayout title="Notifications">
          <div className="flex items-center justify-center h-64">
  ...
  console.log("[AdminNotificationsPage] Rendering main content")
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-muted-foreground">System-wide alerts and administrative notifications</p>
            </div>
